...
msg_types = ['POS', 'VFR_HUD', 'ATT', 'ATTITUDE', 'MODE',
             'HEARTBEAT', 'SYS_STATUS', 'BATTERY_STATUS',
             'BATT', 'HOME', 'HOME_POSITION', 'GPS', 'GPA', 'GPS_RAW_INT',
             'XKF1', 'RCIN', 'ORGN' ]
...
def time_format(tnow):
    ...
    t1 = time.strftime("%Y/%m/%d %H:%M:%S", time.localtime(tnow))
...
while True:
    ...
    if mtype == 'POS':
        ...
        fields['OSD.height'] = m.RelHomeAlt
